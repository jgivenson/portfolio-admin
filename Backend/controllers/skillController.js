const Skill = require('../models/skillModel')

const mongoose = require('mongoose')


//get all the skills
const getSkills = async (req,res)=>{
   
    const user_id = req.user._id;


    const skills = await Skill.find({user_id}).sort({createdAt: -1});
    
    res.status(200).json(skills)
}
//get a single skill
const getSkill = async (req,res) => {
    const { id } = req.params

    if(!mongoose.Types.ObjectId.isValid(id))
    {
        return res.status(404).json({error:"No Such skill"})
    }

    const skill = await Skill.findById(id)
    if (!skill)
    {
        return res.status(404).json({error:"No Such skill"})
    }
    res.status(200).json(skill)
}

// Create new skill
const createSkill = async (req,res)=>{
    const {title,type,description,numberOfYears}=req.body

    let emptyFields =[]

    //check for empty fields
    if(!title)
    {
        emptyFields.push('title')
    }
    if(!type)
    {
        emptyFields.push('type')
    }
    if(!description)
    {
        emptyFields.push('description')
    }
    if(!numberOfYears)
    {
        emptyFields.push('numberOfYears')
    }

    if(emptyFields.length>0)
    {
        return res.status(400).json({error:"Please fill in the missing fields",emptyFields})
    }


    // we are adding the doucment to the db
    try {
        const user_id = req.user._id;

        const skill = await Skill.create({title,type,description,numberOfYears,user_id});
        res.status(200).json(skill)
    } catch (error) {
        res.status(400).json({error:error.message})
    }
}

//delete a skill
const deleteSkill =async (req,res)=>{
    const { id } = req.params
    //check if this is a valid id
    if(!mongoose.Types.ObjectId.isValid(id))
    {
        return res.status(404).json({error:"No Such skill"})
    }
    //delete the skill with the id soecified
    const skill = await Skill.findByIdAndDelete({_id:id})
    if(!skill)
    {
        return res.status(404).json({error:"No Such skill"})
    }

    res.status(200).json(skill)

}

//update skill
const updateSkill = async (req,res)=>{
    const { id } = req.params
    if(!mongoose.Types.ObjectId.isValid(id))
    {
        return res.status(404).json({error:"No such skill"})
    }
    const skill = await Skill.findByIdAndUpdate({_id:id},{
        ...req.body
    })
    if(!skill)
    {
        return res.status(404).json({error:"No Such skill"})
    }
    
        res.status(200).json(skill)

}

module.exports = {
    getSkills,
    getSkill,
    createSkill,
    deleteSkill,
    updateSkill
}